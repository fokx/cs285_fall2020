...
class CQLCritic(BaseCritic):
  ...
  def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
    ...
    assert q_t_values.shape == torch.sum(torch.exp(qa_t_values), dim=1).shape
    # q_t_logsumexp2 = torch.log(
    #   torch.sum(torch.exp(qa_t_values), dim=1) # sum over actions
    # )
    q_t_logsumexp = torch.logsumexp(qa_t_values, dim=-1) # same effect as q_t_logsumexp2
    cql_loss = loss + self.cql_alpha * (
      torch.mean(q_t_logsumexp - q_t_values)
    )
    loss += cql_loss
