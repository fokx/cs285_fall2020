...
class Obstacles(gym.Env, utils.EzPickle): # inherit `utils.EzPickle` is critical for multiprocessing!!
  ...
  def pick_start_pos(self):
    if self.random_starts:
      temp = np.random.uniform([self.boundary_min, self.boundary_min + 1.25],
                               [self.boundary_max - 0.4, self.boundary_max], (self.action_dim,))
